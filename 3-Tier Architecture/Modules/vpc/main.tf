...
resource "aws_nat_gateway" "nat_gat_1" {
  allocation_id = var.nat_allocation_id_1
  subnet_id     = var.nat_sub_id_1
...
resource "aws_nat_gateway" "nat_gat_2" {
  allocation_id = var.nat_allocation_id_2
  subnet_id     = var.nat_sub_id_2
...
resource "aws_route_table" "public_rt" {
  ...
  route {
    ...
    gateway_id = var.route_table_IG_id
  }
  tags = {
    Name = "Public RT"
  }
...
resource "aws_route_table_association" "priv_sub_assoc" {
  subnet_id      = var.rt_assoc_prisub_1
  #gateway_id     = aws_nat_gateway.nat_gat_1.id
  route_table_id = var.rt_assoc_rtid_priv
}

resource "aws_route_table_association" "priv_sub_assoc_1" {
  subnet_id      = var.rt_assoc_prisub_2
  #gateway_id     = aws_nat_gateway.nat_gat_2.id
  route_table_id = var.rt_assoc_rtid_priv_1
}
...
resource "aws_security_group" "DBMS" {
  ...
  ingress {
    ...
    protocol         = var.ingress_sg_protocol
    cidr_blocks      = [aws_vpc.threetiervpc.cidr_block]
    #security_groups  = [aws_security_group.DBMS.id]
    self             = var.self_ip
  }
